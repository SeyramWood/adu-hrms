    ...
    searchKey: function searchKey(nv) {
      ...
      if (nv) {
        ...
        this.$axios.get(url).then(function (_ref) {
          var data = _ref.data;
          _this.searchResults = data.data.map(function (u) {
            ...
            switch (_context2.prev = _context2.next) {
              ...
              case 3:
                ...
                if (data.data.length) {
                  ...
                  if (!intersection.length) {
                    _this3.searchResults = [].concat(_toConsumableArray(_this3.searchResults), _toConsumableArray(data.data.map(function (u) {
                      u.personal_details = JSON.parse(u.personal_details);
                      u.job = u.job ? JSON.parse(u.job) : u.job;
                      return u;
                    })));
